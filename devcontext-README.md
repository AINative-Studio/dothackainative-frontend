...
### About Cody - AINative's CTO Agent

**Cody** is AINative Studio's **authoritative CTO-type AI agent** - an expert-level development assistant that serves as our technical leadership AI. Cody is NOT a third-party tool; it is AINative's proprietary AI agent designed to:

- **Lead technical architecture** decisions
- **Enforce coding standards** and best practices
- **Review code** with CTO-level expertise
- **Guide developers** through complex implementations
- **Maintain project memory** and context
- **Automate development workflows**

The `.ainative/` folder in this repository contains Cody's complete knowledge base and operational guidelines.
...
- **Empower Cody** with complete project context and authority
...
2. **`.ainative/`** - For AINative Cody (our internal CTO agent)
...
│
└── .ainative/                   # AINative Cody (CTO Agent) context
...
- AI attribution in commits (Claude, Anthropic, Cody references)
...
AINative Platform provides comprehensive REST APIs for all development needs. All endpoints require authentication via Bearer token or API key.

### Base URLs

- **Production**: `https://api.ainative.studio`
- **Documentation**: `https://api.ainative.studio/docs` (Interactive Swagger UI)
- **Staging**: `https://staging-api.ainative.studio` (Contact DevOps for access)
...
| `/v1/auth/login` | POST | Login and get JWT tokens |
...
| `/v1/users/me` | PUT | Update current user profile |
| `/v1/users/me/password` | PUT | Change password |
...
| `/v1/api-keys` | POST | Create new API key |
| `/v1/api-keys/{key_id}` | GET | Get API key details |
| `/v1/api-keys/{key_id}` | PUT | Update API key (name, scopes) |
| `/v1/api-keys/{key_id}` | DELETE | Revoke API key |
| `/v1/api-keys/{key_id}/rotate` | POST | Rotate API key secret |

**Admin endpoints** (requires admin role):
- `/admin/api-keys` - List all API keys across all users
- `/admin/api-keys/{user_id}` - Manage user's API keys

#### Organizations

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/v1/organizations` | GET | List your organizations |
| `/v1/organizations` | POST | Create new organization |
| `/v1/organizations/{org_id}` | GET | Get organization details |
| `/v1/organizations/{org_id}` | PUT | Update organization |
| `/v1/organizations/{org_id}/members` | GET | List organization members |
| `/v1/organizations/{org_id}/members` | POST | Add member to organization |
| `/v1/organizations/{org_id}/members/{user_id}` | DELETE | Remove member |

#### Agent Coordination & Orchestration

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/v1/agents` | GET | List agent instances |
| `/v1/agents` | POST | Create agent instance |
| `/v1/agents/{agent_id}` | GET | Get agent details |
| `/v1/tasks` | POST | Create agent task |
| `/v1/tasks/{task_id}/execute` | POST | Execute task |
| `/v1/tasks/{task_id}` | GET | Get task status |
| `/v1/messages` | POST | Send message to agent |
| `/v1/messages/{agent_id}` | GET | Get agent messages |
| `/v1/sequences` | POST | Create task sequence |
| `/v1/sequences/{sequence_id}/execute` | POST | Execute sequence |
| `/v1/agents/workload` | GET | Get agent workload metrics |

#### Memory & Context Management

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/v1/memories` | POST | Store agent memory |
| `/v1/memory/{memory_id}` | GET | Retrieve memory |
| `/v1/memory/{memory_id}` | PUT | Update memory |
| `/v1/memory/{memory_id}` | DELETE | Delete memory |
| `/v1/memories/search` | POST | Semantic memory search |

#### A/B Testing & Experiments

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/v1/tests` | GET | List A/B tests |
| `/v1/tests` | POST | Create A/B test |
| `/v1/tests/{test_id}` | GET | Get test details |
| `/v1/tests/{test_id}/start` | POST | Start test |
| `/v1/tests/{test_id}/analyze` | GET | Analyze test results |
| `/v1/tests/{test_id}/optimize` | POST | Auto-optimize test |
| `/v1/tests/{test_id}/statistics` | GET | Get test statistics |
| `/v1/tests/{test_id}/export` | GET | Export test data |

#### Code Quality & PR Analysis

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/v1/pr-analysis` | POST | Analyze pull request |
| `/v1/pr-analysis/{pr_id}` | GET | Get PR analysis results |
| `/v1/code-quality/check` | POST | Run code quality checks |
| `/v1/code-quality/metrics` | GET | Get code quality metrics |

#### Feedback & Learning (RLHF)

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/v1/interactions` | POST | Log user interaction |
| `/v1/interactions/{id}/feedback` | POST | Submit feedback on interaction |
| `/v1/feedback/user/{user_id}` | GET | Get user feedback history |
| `/v1/performance/agent/{agent_id}` | GET | Get agent performance metrics |
| `/v1/insights` | GET | Get learning insights |
| `/v1/learning-tasks` | POST | Create learning task |
...
| `/v1/version` | GET | Get API version info |
| `/v1/status` | GET | Get system status |
...
curl -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  https://api.ainative.studio/v1/users/me
```
...
**Option 3: API Key in Header (Alternative)**
```bash
...
curl -X POST https://api.ainative.studio/v1/api-keys \
  ...
  -d '{
    ...
    "scopes": ["read:vectors", "write:vectors", "read:tables"],
...
```json
{
  "id": "key_abc123",
  "name": "Development Key",
  "key": "ak_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
  "scopes": ["read:vectors", "write:vectors", "read:tables"],
